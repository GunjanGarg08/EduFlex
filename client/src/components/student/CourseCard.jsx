...
const CourseCard = ({ course }) => {
    ...
    const rating = typeof calculateRating === "function" ? calculateRating(course) : 0
    const price = typeof course.coursePrice === "number" ? course.coursePrice : 0
    const discount = typeof course.discount === "number" ? course.discount : 0
    const finalPrice = price - (discount * price / 100)

    return (
        <Link onClick={() => scrollTo(0, 0)} to={'/course/' + course._id} className="border border-gray-500/30 pb-6 overflow-hidden rounded-lg">
            <img className="w-full" src={course.courseThumbnail || assets.defaultThumbnail} alt='' />
            <div className="p-3 text-left">
                ...
                <p className="text-base font-semibold text-gray-800">{currency}{finalPrice.toFixed(2)}</p>
            </div>
        </Link>
    )
}
